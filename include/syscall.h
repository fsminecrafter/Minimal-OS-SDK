...
// Returned when this program (always PROC_PRIVILEGE_USER when loaded
// via `run`) calls a syscall operation reserved for the kernel - e.g.
// SYS_MANAGER init/control ops, or SYS_USB_INIT. Not currently
// returned for anything else; safe to treat like any other failure
// code (check for it, don't retry the same call).
